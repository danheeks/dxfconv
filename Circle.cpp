...
CircleOrLine::CircleOrLine(const Point& p0, const Point& p1, const Point& p2)
{
	...
	gp_Lin line = make_line(gp_Pnt(p0.x, p0.y, 0.0), gp_Pnt(p2.x, p2.y, 0.0));
	if (line.Contains(gp_Pnt(p1.x, p1.y, 0.0), TOLERANCE))
	{
		m_is_a_line = true;
		m_p0 = p0;
		m_p1 = p2;
...
bool CircleOrLine::PointIsOn(const Point& p, double accuracy)
{
	if (m_is_a_line)
	{
		gp_Lin line = make_line(gp_Pnt(m_p0.x, m_p0.y, 0.0), gp_Pnt(m_p1.x, m_p1.y, 0.0));
		return line.Contains(gp_Pnt(p.x, p.y, 0.0), accuracy) == Standard_True;
	}
...
bool CircleOrLine::LineIsOn(const Point& p0, const Point& p1, double accuracy)
{
	...
	if (this->m_is_a_line)
	{
		...
		gp_Lin line_this = make_line(gp_Pnt(m_p0.x, m_p0.y, 0.0), gp_Pnt(m_p1.x, m_p1.y, 0.0));
		gp_Lin line = make_line(gp_Pnt(p0.x, p0.y, 0.0), gp_Pnt(p1.x, p1.y, 0.0));
		return (line.Direction() * line_this.Direction() >= -0.0000000001); // they are going in the same direction
	}
